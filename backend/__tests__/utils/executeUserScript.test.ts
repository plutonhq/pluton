...
describe('executeUserScript', () => {
	...
	describe('extension-based execution', () => {
		const extensionCases = [
			...
			{ ext: '.sh', platform: 'win32', expectedExe: 'bash', scriptPath: '/scripts/backup.sh' },
			{
				ext: '.bash',
				platform: 'linux',
				expectedExe: '/bin/bash',
				scriptPath: '/scripts/backup.bash',
			},
			{
				ext: '.ps1',
				platform: 'win32',
				expectedExe: 'powershell.exe',
				scriptPath: '/scripts/backup.ps1',
			},
	...
	describe('spawn options', () => {
		...
		it('passes timeout option to spawn in milliseconds', async () => {
			const child = freshChild();
			...
			expect(mockSpawn).toHaveBeenCalledWith(
				expect.any(String),
				expect.any(Array),
				expect.objectContaining({ timeout: 3000000 })
			);
		});
	});
	...
	it('rejects on SIGTERM (timeout)', async () => {
		...
		await expect(promise).rejects.toThrow('Script timed out after 5000s');
	});
	...
	it('uses default timeout message when timeout is not set', async () => {
		...
		await expect(promise).rejects.toThrow('Script timed out after 60s');
	});
