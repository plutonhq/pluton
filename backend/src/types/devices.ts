...
export interface DeviceDependencyVersions {
	restic: string;
	rclone: string;
	agent: string;
}
...
export interface DiscoveredFilesystem {
	device: string; // e.g., "vda4"
	mountpoint: string; // e.g., "/"
	fstype: string | null; // e.g., "ext4"
	label: string | null;
	uuid: string | null;
	size: string; // e.g., "9GB"
	isCritical: boolean; // The flag that determines if it's auto-selected.
	used?: string; // e.g., "4GB"
}
