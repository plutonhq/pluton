...
load_existing_credentials() {
    ...
    local enc_env_file="${CONFIG_DIR}/pluton.enc.env"
...
write_configuration() {
    ...
    # Clean up legacy encryption key file from data directory
    if [ -f "${DATA_DIR}/pluton.enc.env" ]; then
        rm -f "${DATA_DIR}/pluton.enc.env"
        echo "  Removed legacy encryption key file from ${DATA_DIR}/pluton.enc.env"
    fi
