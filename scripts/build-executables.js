...
import { readdir, readFile, writeFile, mkdir, copyFile, rm, access, chmod } from 'fs/promises';
	...
	for (const [platform, config] of Object.entries(targets)) {
		...
		try {
			...
			try {
				...
				console.warn(`  ⚠️  tar command failed, trying alternative extraction...`);
			...
			console.warn(`  ⚠️  Failed to download better-sqlite3 for ${platform}: ${error.message}`);
			console.warn(`  You may need to build it manually or copy from node_modules`);
		}
	}
		...
		try {
			...
			if (await fileExists(resticPath)) {
				...
				if (config.resticUrl.endsWith('.bz2')) {
					// Decompress bz2 (Unix)
					try {
						execSync(`bunzip2 -c "${resticTemp}" > "${resticPath}"`, { stdio: 'pipe' });
						await rm(resticTemp, { force: true });
					} catch (err) {
						console.warn(`  ⚠️  bunzip2 failed, file saved at: ${resticTemp}`);
					}
					...
					try {
						...
						for (const entry of zipEntries) {
							if (entry.entryName.endsWith('.exe') && entry.name.includes('restic')) {
								zip.extractEntryTo(entry, dirname(resticPath), false, true, false, resticName);
								break;
							}
						}
						await rm(resticTemp, { force: true });
					} catch (err) {
						console.warn(`  ⚠️  zip extraction failed: ${err.message}`);
						console.warn(`  File saved at: ${resticTemp}`);
					}
				}
				
				// Set executable permissions on Unix
				if (!isWindows && os.platform() !== 'win32' && await fileExists(resticPath)) {
			...
			console.warn(`  ⚠️  Failed to download restic: ${error.message}`);
		}
		...
		try {
			...
			if (await fileExists(rclonePath)) {
				...
				try {
					...
					for (const entry of zipEntries) {
						if (!entry.isDirectory && (entry.name === 'rclone.exe' || entry.name === 'rclone')) {
							zip.extractEntryTo(entry, dirname(rclonePath), false, true, false, rcloneName);
							break;
						}
					}
					...
					console.warn(`  ⚠️  zip extraction failed: ${err.message}`);
					console.warn(`  File saved at: ${rcloneTemp}`);
				}
				
				// Set executable permissions on Unix
				if (!isWindows && os.platform() !== 'win32' && await fileExists(rclonePath)) {
			...
			console.warn(`  ⚠️  Failed to download rclone: ${error.message}`);
		}
	}
...
async function generateExecutables() {
	...
	const pkgTargets = Object.values(targets).map(t => t.pkgTarget).join(',');
...
async function createDistributionPackages() {
	...
	for (const [platform, config] of Object.entries(targets)) {
		...
		const packageBinariesDir = join(packageDir, 'binaries', config.binaryPlatform);
		...
		if (await fileExists(sqliteAssetsDir)) {
			...
			// Our downloaded tarball might have a different structure, so we need to be careful.
			// The tarball usually extracts to build/Release/better_sqlite3.node directly or similar.
			
			const sqliteDestDir = join(packageDir, 'node_modules', 'better-sqlite3', 'build', 'Release');
			await mkdir(sqliteDestDir, { recursive: true });
			
			// We copy the contents of the extracted folder to the destination
			// If the extracted folder has build/Release structure, we need to handle that.
			// Based on the setupBetterSqlite3 function, we extract to platformAssetsDir.
			// Let's check if we need to drill down.
			
			// Check if build/Release exists in source
			const sourceBuildRelease = join(sqliteAssetsDir, 'build', 'Release');
			if (await fileExists(sourceBuildRelease)) {
				await copyDirectory(sourceBuildRelease, sqliteDestDir);
			} else {
				// Fallback: copy everything from root of extraction
				await copyDirectory(sqliteAssetsDir, sqliteDestDir);
			}
		...
		if (await fileExists(sourceBinariesDir)) {
			...
			console.warn(`  ⚠️  Binaries not found for ${config.binaryPlatform} at ${sourceBinariesDir}`);
		}
...
async function main() {
	try {
		...
		console.log(`📁 Distribution packages: ${join(rootDir, 'dist', 'executables')}\n`);
		
		console.log('📋 Next steps:');
		console.log('   1. Test executables on target platforms');
		console.log('   2. Create installers (Windows Inno Setup, Linux/macOS scripts)');
